...
import os
import json
from cloudrender.render import SimpleMesh, DirectionalLight
...
def load_mesh(scene, camera, path_input_data):
	renderable_mesh = SimpleMesh(camera=camera)
	renderable_mesh.generate_shadows = True
	renderable_mesh.init_context()
	mesh = trimesh.load(path_input_data)
	renderable_mesh.set_buffers(mesh)
	scene.add_object(renderable_mesh)
...
def main_drawing_loop(resolution, fps, video_start_time, video_length_seconds):
	...
	load_mesh(main_scene, camera, '{}/pointcloud_mesh.ply'.format(args.path_input_data))
